...
def up_select_file(fl):
    ...
    xmlfile.delete(0,len(xmlfile.get()))
    xmlfile.insert(0,fl)
...
def extract(buff):
    ...
    # чтение списка каталогов и файлов
    n = 0
    f_list = []
...
def show_window():
    ...
    backup = ''
    for f in os.listdir('.'):
        if (f[:8] == 'backup.2' or f[:10] == 'rutracker-'):
            backup = f
